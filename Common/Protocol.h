...
constexpr quint32 MAX_JSON_MESSAGE_BYTES = 16 * 1024 * 1024; // 容纳 8MB Base64 小文件
...
namespace MsgType {
    ...
    // 房间密码
    inline const QString SET_ROOM_PASSWORD_REQ  = QStringLiteral("SET_ROOM_PASSWORD_REQ");
    inline const QString SET_ROOM_PASSWORD_RSP  = QStringLiteral("SET_ROOM_PASSWORD_RSP");
...
/// 从缓冲区解析一个完整 JSON 帧，并区分“待补全”与“必须拒绝”。
inline FrameParseResult inspectFrame(QByteArray &buffer, QJsonObject &msg) {
    if (buffer.size() < 4)
...
/// 旧客户端保持 bool 语义；服务端使用 inspectFrame 处理拒绝原因。
inline bool unpack(QByteArray &buffer, QJsonObject &msg) {
    return inspectFrame(buffer, msg) == FrameParseResult::Complete;
}
