...
test('supports keyboard access to files, profiles, retry, and message actions', () => {
  ...
  assert.match(messages, /role="menu" :aria-label="actionMessages\.menu"/)
...
test('labels composer controls and honors focus and reduced-motion preferences', () => {
  assert.match(input, /role="toolbar" :aria-label="messages\.toolbar"/)
  assert.match(input, /role="progressbar"/)
  assert.match(input, /:aria-label="messages\.recoveryStatus"/)
  assert.match(input, /messages\.reselectPrefix/)
  assert.match(input, /messages\.cancelSendPrefix/)
  assert.match(input, /:aria-label="messages\.messageContent"/)
  assert.match(input, /:aria-label="messages\.messageBytes"/)
