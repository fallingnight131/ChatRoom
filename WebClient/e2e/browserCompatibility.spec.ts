...
test("switches the authenticated shell locale from profile and persists it", async ({ page }) => {
  ...
  const forwardAction = messageActions.getByRole("menuitem", { name: "Forward to other conversations" });
  await expect(forwardAction).toBeVisible();
  await forwardAction.click();
  await expect(messageActions).toBeHidden();
  const forwardDialog = page.getByRole("dialog", { name: "Forward to other conversations" });
  await expect(forwardDialog).toBeVisible();
  await expect(forwardDialog.getByRole("tablist", { name: "Forwarding target type" })).toBeVisible();
  await expect(forwardDialog.getByRole("tab", { name: /Friends/ })).toBeVisible();
  await expect(forwardDialog.getByRole("button", { name: "Confirm forwarding" })).toBeDisabled();
  await page.keyboard.press("Escape");
