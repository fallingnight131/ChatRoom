...
/** Bounded deterministic profile-image cleanup delay policy. */
public final class ProfileImageCleanupBackoff {
    ...
    public ProfileImageCleanupBackoff(Duration healthyInterval,
            Duration initialFailureDelay, Duration maximumFailureDelay) {
        this.healthyInterval = bounded(healthyInterval, "healthyInterval",
                Duration.ofSeconds(10), Duration.ofDays(1));
        this.initialFailureDelay = bounded(initialFailureDelay, "initialFailureDelay",
                Duration.ofSeconds(1), Duration.ofHours(1));
        this.maximumFailureDelay = bounded(maximumFailureDelay, "maximumFailureDelay",
                initialFailureDelay, Duration.ofHours(6));
    }
    ...
    public Duration delay(int consecutiveFailures) {
        ...
        if (consecutiveFailures == 0) return healthyInterval;
