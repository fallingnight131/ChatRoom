...
| Repeated expensive password work | Account and room-password fields are capped; connection, account/room, direct-peer IP, and process/gateway windows bound work before hashing. The detached Java join handler consumes the shared admission budget before Argon2 and returns a bounded retry hint. | V1 state is process-local and direct-peer IP may aggregate a proxy; use trusted proxy identity and shared Redis enforcement only with the future gateway design |
