...
gate requires the repository/ViewModel wiring in `ChatWindow`. The dialog also
requires an explicit label buddy, selector description, deterministic first-tab
transition to Refresh, and visible write-failure rollback; that failure emits a
...
The conversation header and room-member sidebar extend that gate. Titles,
administrator/presence labels, room settings and accessible member semantics
derive from stable roles through the catalog. Locale projection may replace a
...
Chinese-to-English tabs, actions, validation, exact restart persistence, opaque
socket-detail retention across recomposition, and failed-write selector
...
an opaque field, protocol retryability to a locale-independent failure, and
same-account retry to one operation ID. The static composition gate rejects
controller-manufactured Chinese “server detail”.
...
stable account, distinguishes localized retryable failure from opaque safe
detail, confirms one unblock command, and proves disconnect keeps rows visible
while disabling network actions. The custom default-No confirmation is source-
locked; native Windows modal focus and screen-reader evidence remains a product
gate.
