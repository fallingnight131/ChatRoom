...
real older executable/schema compatibility, downgrade policy, Authenticode,
timestamping, updater orchestration, or Windows 10/11 clean-host behavior. Those
remain M4 gates. Hosted Windows Server CI is configured for the synthetic path,
but the workflow result is not claimed until it runs remotely.
